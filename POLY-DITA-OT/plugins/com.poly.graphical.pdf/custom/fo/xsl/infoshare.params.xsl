...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" version="2.0">
  ...
  <xsl:param name="DRAFT" />
   <!-- for local dev only BEGIN -->
   <!--<xsl:param name="WORKDIR" select="'C:\suite-proj\Plantronics\source\01-07-2015\en\workset'"></xsl:param>-->
   <!-- for local dev only END -->
  ...
  <!-- ============================================================== -->
   ...
   <xsl:param name="multilingual">
      ...
      <xsl:value-of select="translate($param, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')"/>      
	</xsl:param>
   ...
   <xsl:param name="combine-language-string">
      <xsl:for-each
         select="document($jobTicket,/)/job-specification/parameter[@name='combine-language']">
         <!--<xsl:sort> </xsl:sort>-->
         <xsl:value-of select="."/>
         <xsl:text> </xsl:text>
      </xsl:for-each>
