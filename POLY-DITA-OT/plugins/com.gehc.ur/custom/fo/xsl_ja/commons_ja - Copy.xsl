...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="startPageNumbering">
        ...
        <!--GEHC. Commented out the if statement to keep page restarting from being limited to just first chapter in book-->
...
<!--                    <xsl:apply-templates select="*[not(contains(@class, ' topic/topic '))]"/>-->
                            ...
                            <xsl:with-param name="theParameters">								
								<number>
                                    ...
                                    <fo:inline xsl:use-attribute-sets="__chapter__frontmatter__number__container">
									    <!--LWW: this creates a marker with the current chapter number to be used elsewhere-->
                                        <fo:marker marker-class-name="current-chapter-number">
                                        <xsl:value-of select="$chapterNumber"/></fo:marker>
                                        <xsl:value-of select="$chapterNumber"/><xsl:text> </xsl:text>
                                    </fo:inline>
									<fo:inline xsl:use-attribute-sets="__chapter__frontmatter__title">
                    ...
                    <fo:table-cell>-->
                        <fo:block xsl:use-attribute-sets="__toc__mini">
                            <xsl:if test="*[contains(@class, ' topic/topic ')]">
								...
								<fo:block-container width="2in" margin-top="3pt">
                                ...
                                <fo:block xsl:use-attribute-sets="__toc__mini__list">
    ...
    <xsl:template match="*[contains(@class, ' topic/topic ')]" mode="in-this-chapter-list">
        <!--<fo:list-item xsl:use-attribute-sets="ul.li">
            ...
            <!--<fo:list-item-body xsl:use-attribute-sets="ul.li__body">-->
                <fo:block xsl:use-attribute-sets="xref__section">
                    <fo:basic-link internal-destination="{@id}">
                        <fo:inline><xsl:value-of select="child::*[contains(@class, ' topic/title ')]"/></fo:inline>
                        <!--<fo:inline>
						    <fo:leader xsl:use-attribute-sets="__mini__toc__leader"/>
						    <fo:page-number-citation ref-id="{concat('_OPENTOPIC_TOC_PROCESSING_', generate-id())}"/>
						</fo:inline>-->
						<!--GEHC. Adds chapter number prefix to page number-->
						<fo:inline>
						    <fo:leader xsl:use-attribute-sets="__mini__toc__leader"/>
    ...
    <xsl:template match="*" mode="processTopicTitle">
        ...
        <fo:block>
            ...
            <fo:block>
				...
				<xsl:if test="$level = 3">
                    <fo:marker marker-class-name="current-h3">
                        <xsl:call-template name="getTitle"/>
                    </fo:marker>
                </xsl:if>
				<xsl:if test="$level = 4">
                    <fo:marker marker-class-name="current-h4">
                        <xsl:call-template name="getTitle"/>
                    </fo:marker>
                </xsl:if>
	...
	<!--GEHC. Added to force formatting of title in notices-->
	<xsl:template match="*[contains(@class,' topic/title ')][@outputclass = 'notice']">
        <fo:block xsl:use-attribute-sets="topic.topic.title" id="{@id}">
            <xsl:call-template name="getTitle"/>
        </fo:block>
    </xsl:template>
    ...
    <xsl:template match="*[contains(@class,' topic/fig ')]/*[contains(@class,' topic/title ')]">
        <fo:block xsl:use-attribute-sets="fig.title">
            <xsl:call-template name="commonattributes"/>
			<!--GEHC. Template "getChapterPrefix" lives in custom.xsl-->
			<xsl:variable name="chapPrefix">
                <xsl:call-template name="getChapterPrefix"/>
            </xsl:variable>
            <xsl:call-template name="insertVariable">
                ...
                <xsl:with-param name="theParameters">
                    <number>
						<xsl:call-template name="getChapterPrefix" />
						<xsl:text>-</xsl:text>
						<!--GEHC. Starts figure number over at each chapter/appendix-->
						<xsl:choose>
							<!-- jmcneal 2012-05-04 -->
							...
							<xsl:otherwise>
								<xsl:value-of select="count(./preceding::*[contains(@class, ' topic/fig ')][child::*[contains(@class, ' topic/title ')]][ancestor-or-self::*[contains(@class, ' topic/topic')][position()=last()]])+1"/>
							</xsl:otherwise>
						</xsl:choose>
                    </number>
                    <title>
	...
	<!--GEHC. Processes titles in figgroup identically to those directly in fig-->
            ...
            <xsl:call-template name="insertVariable">
                ...
                <xsl:with-param name="theParameters">
                    <number>
						...
						<xsl:text>-</xsl:text>
						<!--GEHC. Starts figure number over at each chapter/appendix-->
                         <!--xsl:number level="any" count="*[contains(@class, ' topic/figgroup ')][child::*[contains(@class, ' topic/title ')]]" from="/bookmap/*/chapter"/-->
						 <xsl:choose>
							<!-- jmcneal 2012-05-04 -->
							<xsl:when test="count(ancestor-or-self::*[contains(@class, ' topic/topic')][position()=last()][count(preceding-sibling::*[contains(@class, ' topic/topic')]) &gt; 0])">
								<xsl:value-of select="count(./preceding::*[contains(@class, ' topic/figgroup ')][child::*[contains(@class, ' topic/title ')]][ancestor-or-self::*[contains(@class, ' topic/topic')][position()=last()]]) - count(ancestor-or-self::*[contains(@class, ' topic/topic')][position()=last()]/preceding-sibling::*[contains(@class, ' topic/topic')]//*[contains(@class, ' topic/figgroup ')][child::*[contains(@class, ' topic/title ')]])+1"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="count(./preceding::*[contains(@class, ' topic/figgroup ')][child::*[contains(@class, ' topic/title ')]][ancestor-or-self::*[contains(@class, ' topic/topic')][position()=last()]])+1"/>
							</xsl:otherwise>
						</xsl:choose>
                    </number>
                    <title>
...
<!--             add keycol here once implemented-->
    ...
    <!-- These values specify the length of a short description that will
        ...
        <xsl:choose>
            <xsl:when test="$locale = 'en_US' or $locale = 'fr_FR' or $locale = 'EN' or $locale = 'FR'">
                <xsl:value-of select="360"/>
            </xsl:when>
            <xsl:when test="$locale = 'ja_JP' or $locale = 'JA'">
                <xsl:value-of select="141"/>
            </xsl:when>
            <xsl:when test="$locale = 'zh_CN' or $locale = 'ZH-CN'">
                <xsl:value-of select="141"/>
            </xsl:when>
...
-->
    ...
    <xsl:template match="*[contains(@class, ' topic/p ')]">
		<!--GEHC. This applies a different attribute set to p within entry-->
		<xsl:choose>
		    <xsl:when test="ancestor::*[contains(@class, ' topic/entry ')]">
                <fo:block xsl:use-attribute-sets="p__table" id="{@id}">
                    <xsl:apply-templates/>
                </fo:block>
            </xsl:when>	

            <!--GEHC. If the p is the last p within a note that is within info, stepxmp or stepresult, 
			add the extra block to force correct spacing and apply a different attribute set. This results in the extra
			block also being added to notes that are the very last element in a task, which results in a little extra space
			before the next title.-->
			<xsl:when test="ancestor::*[contains(@class, ' task/info ')] or ancestor::*[contains(@class, ' task/stepxmp ')] or ancestor::*[contains(@class, ' task/stepresult ')]">
			    <xsl:if test="parent::*[contains(@class, ' topic/note ')]">
				<xsl:choose>
				    <xsl:when test="count(following-sibling::*)=0">
					    <fo:block xsl:use-attribute-sets="p__note" id="{@id}">
                            <xsl:apply-templates/>
                        </fo:block>
				        <!--<fo:block font-size="2pt" color="#ffffff" space-above="0pt" space-below="0pt">-->
						<fo:block font-size="2pt" color="#ffffff" margin-top="0pt" margin-bottom="0pt">
                            <xsl:text>--</xsl:text>
                        </fo:block>
					</xsl:when>
					<xsl:otherwise>
				...
				<!--GEHC. If the p is within info, stepxmp, stepresult but not within a note format as plain p-->
				<xsl:if test="parent::*[contains(@class, ' task/info ')]">
				    <fo:block xsl:use-attribute-sets="info">
                        <xsl:call-template name="commonattributes"/>
                        <xsl:apply-templates/>
                    </fo:block>
			    </xsl:if>
				<xsl:if test="parent::*[contains(@class, ' task/stepxmp ')]">
				    <fo:block xsl:use-attribute-sets="stepxmp">
                        <xsl:call-template name="commonattributes"/>
                        <xsl:apply-templates/>
                    </fo:block>
			    </xsl:if>
				<xsl:if test="parent::*[contains(@class, ' task/stepresult ')]">
			...
			<!--<xsl:when test="ancestor::*[contains(@class, ' task/stepxmp ')]">
			    <xsl:if test="parent::*[contains(@class, ' topic/note ')]">
				<xsl:choose>
				    <xsl:when test="count(following-sibling::*)=0">
					    <fo:block xsl:use-attribute-sets="p__note" id="{@id}">
			...
			<xsl:when test="ancestor::*[contains(@class, ' task/stepresult ')]">
			    <xsl:if test="parent::*[contains(@class, ' topic/note ')]">
				<xsl:choose>
				    <xsl:when test="count(following-sibling::*)=0">
					    <fo:block xsl:use-attribute-sets="p__note" id="{@id}">
			...
			<!--GEHC. If the p is within li within ul within note, apply a different attribute set-->
			<xsl:when test="ancestor::*[contains(@class,' topic/note ')]/*[contains(@class,' topic/ul ')]/*[contains(@class,' topic/li ')]">
				<fo:block xsl:use-attribute-sets="p__li__note" id="{@id}">
                    <xsl:apply-templates/>
                </fo:block>
			</xsl:when>
			
			<!--GEHC. If the p is within note, apply a different attribute set-->
    ...
    <xsl:template match="*" mode="placeNoteContent">
	    <!--GEHC. Count preceding siblings within section, context, prereq.-->
		<xsl:variable name="sibling-count-section" select="number(count(../preceding-sibling::*[contains(@class, ' topic/section ')]))"/>
		<!--GEHC. Count preceding siblings within conbody, refbody. Don't need this now.
		<xsl:variable name="sibling-count-body" select="number(count(../preceding-sibling::*[contains(@class, ' topic/body ')]))"/>-->
		    <!--GEHC. If note is first in section, context, prereq, insert block-->
			<!--<xsl:if test="$sibling-count-section = 0">
		        <fo:block color="#ffffff" font-size="1pt" id="" keep-with-next.within-page="always" 
				keep-with-previous.within-page="always" line-height="6pt" space-after="0in" space-before="0in">.</fo:block>
		    </xsl:if>-->
        <fo:block xsl:use-attribute-sets="note">
            <fo:float float="start">
			<fo:block-container xsl:use-attribute-sets="block-container.note">
			<xsl:call-template name="commonattributes"/>
                <xsl:choose>
                    <xsl:when test="@type='note' or not(@type)">
                        <fo:block xsl:use-attribute-sets="note__label__note">
                            <xsl:call-template name="insertVariable">
                    ...
                    <xsl:when test="@type='notice'">
                        <fo:block xsl:use-attribute-sets="note__label__notice">
                            <xsl:call-template name="insertVariable">
                    ...
                    <xsl:when test="@type='tip'">
                        <fo:block xsl:use-attribute-sets="note__label__tip">
                            <xsl:call-template name="insertVariable">
                    ...
                    <xsl:when test="@type='fastpath'">
                        <fo:block xsl:use-attribute-sets="note__label__fastpath">
                            <xsl:call-template name="insertVariable">
                    ...
                    <xsl:when test="@type='restriction'">
                        <fo:block xsl:use-attribute-sets="note__label__restriction">
                            <xsl:call-template name="insertVariable">
                    ...
                    <xsl:when test="@type='important'">
                        <fo:block xsl:use-attribute-sets="note__label__important">
                            <xsl:call-template name="insertVariable">
                    ...
                    <xsl:when test="@type='remember'">
                        <fo:block xsl:use-attribute-sets="note__label__remember">
                            <xsl:call-template name="insertVariable">
                    ...
                    <xsl:when test="@type='attention'">
                        <fo:block xsl:use-attribute-sets="note__label__attention">
                            <xsl:call-template name="insertVariable">
                    ...
                    <xsl:when test="@type='caution'">
                        <fo:block xsl:use-attribute-sets="note__label__caution">
                            <xsl:call-template name="insertVariable">
                    ...
                    <xsl:when test="@type='danger'">
                        <fo:block xsl:use-attribute-sets="note__label__danger">
                            <xsl:call-template name="insertVariable">
                    ...
                    <xsl:when test="@type='warning'">
                        <fo:block xsl:use-attribute-sets="note__label__warning">
                            <xsl:call-template name="insertVariable">
                                <xsl:with-param name="theVariableID" select="'Warning'"/>
                            </xsl:call-template>
                        </fo:block>
                    </xsl:when>
                    <!--GEHC. The following 2 when statements are specific to GEHC usage of @othertype for warnings pre DITA 1.2-->					
					<xsl:when test="@type='other'">
                        <xsl:choose>
						    ...
						    <xsl:when test="@othertype='warnicon'">
                                <fo:block xsl:use-attribute-sets="note__label__warnicon">
								    <xsl:call-template name="insertVariable">
                                        <xsl:with-param name="theVariableID" select="'Unordered List bullet'"/>
                                    </xsl:call-template>
                                </fo:block>
                            </xsl:when>
							<!--GEHC. This inserts any other value of @othertype that might have been used-->
							<xsl:otherwise>
							    <fo:block xsl:use-attribute-sets="note__label__other">
                                    <xsl:choose>
                    ...
                    <fo:basic-link>
                        ...
                        <xsl:choose>
                            <xsl:when test="@reftitle">
                                <xsl:text>「</xsl:text><xsl:value-of select="@reftitle"/><xsl:text>」</xsl:text>
                            </xsl:when>
