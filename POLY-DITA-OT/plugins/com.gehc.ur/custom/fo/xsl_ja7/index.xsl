...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="createIndex">
    <xsl:if test="//opentopic-index:index.groups//opentopic-index:index.entry and $index-entries//opentopic-index:index.entry">
      <!--GEHC.Starts the index on page 1-->
	  <fo:page-sequence master-reference="index-sequence" initial-page-number="1" xsl:use-attribute-sets="__force__page__count">
  ...
  <xsl:template match="opentopic-index:index.entry">
    <xsl:if test="opentopic-index:refID/@value">
        ...
        <xsl:choose>
            <!--GEHC.Commented out "when" processing to eliminate top-level index page entries. 
		    This creates error messages in log but does not affect formatting or function-->
				  <xsl:when test="opentopic-index:index.entry">
  ...
  <xsl:param name="useFrameIndexMarkup" select="'true'"/> <!--GEHC-->
      ...
      <xsl:if test="not($no-page)">
        <xsl:if test="$idxs">
          ...
          <fo:inline color="#000000" font-style="normal">
              <xsl:for-each select="$idxs">
				  <!--GEHC. Adds chapter number to page number-->
				  <!-- jmcneal 2012-05-08 -->
				  <xsl:for-each select="//opentopic-index:refID[@value = $idxs/@value][not(ancestor::opentopic-index:index.groups)]">
						<fo:basic-link internal-destination="{./ancestor::*[@id][1]/@id}">
							<xsl:call-template name="getChapterPrefixForIndex">
								<xsl:with-param name="currentNode" select="./ancestor::*[@id][1]/@id"/>
							</xsl:call-template>
							<xsl:text>-</xsl:text>
							<fo:page-number-citation ref-id="{./ancestor::*[@id][1]/@id}"/>
						</fo:basic-link>
						<xsl:if test="following::opentopic-index:refID[@value = $idxs/@value][not(ancestor::opentopic-index:index.groups)]"><xsl:text>, </xsl:text></xsl:if>
					</xsl:for-each>
				</xsl:for-each>
		  </fo:inline>
        </xsl:if>
