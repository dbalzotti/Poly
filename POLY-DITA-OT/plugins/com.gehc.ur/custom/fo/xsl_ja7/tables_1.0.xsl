...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!--GEHC.This indents tables within "incol" figs 2.25in. Tables within "fullpg" figs are not indented.
	Tables not within figs are indented 2.25in-->
	<xsl:template match="*[contains(@class, ' topic/table ')]">
		...
		<xsl:choose>
			<xsl:when test="ancestor::*[contains(@class, ' topic/fig ')]/@outputclass='incol'">
			    <!--<fo:block-container start-indent="2.25in">-->
				<fo:block-container xsl:use-attribute-sets="table__incol__indent">
			...
			<xsl:when test="ancestor::*[contains(@class, ' topic/fig ')]/@outputclass='fullpg'">
			    <fo:block-container start-indent="0in">
		            <fo:block xsl:use-attribute-sets="table">
                        <xsl:call-template name="commonattributes"/>
                            <xsl:if test="not(@id)">
                                <xsl:attribute name="id">
			...
			<!--GEHC.This section determines indent on tables not within figs, based on @pgwide.-->
			<xsl:otherwise>
			    <xsl:choose>
				    <xsl:when test="@pgwide='1'">
					    <!--GEHC.This forces a "reset" of the block to cancel out any start-indent the table might inherit-->
						<fo:block color="#ffffff" font-size="1pt" id="" keep-with-next.within-page="always" keep-with-previous.within-page="always" line-height="6pt" space-after="0in" space-before="0in">.</fo:block>
						<fo:block-container start-indent="0in" padding-top=".25in">
		                    <fo:block xsl:use-attribute-sets="table">
                                <xsl:call-template name="commonattributes"/>
                                <xsl:if test="not(@id)">
                                    <xsl:attribute name="id">
					...
					<xsl:when test="@pgwide='0' or not(@pgwide)">	
					    <!--<fo:block-container start-indent="2.25in" padding-top=".25in">-->
						<fo:block-container xsl:use-attribute-sets="table__incol__indent" padding-top=".25in">
		                    <fo:block xsl:use-attribute-sets="table">
                                <xsl:call-template name="commonattributes"/>
                                    <xsl:if test="not(@id)">
                                        <xsl:attribute name="id">
	...
	<fo:table-cell xsl:use-attribute-sets="tbody.row.entry">
        ...
        <xsl:call-template name="applyAlignAttrs"/>
        <xsl:call-template name="generateTableEntryBorder"/>
	    <!--GEHC.Count the number of rows before the current row-->
		<xsl:variable name="preceding-row-count" select="number(count(../preceding-sibling::*[contains(@class, ' topic/row ')]))"/>
		<!--GEHC.Count the number of rows after the current row-->
		<xsl:variable name="following-row-count" select="number(count(../following-sibling::*[contains(@class, ' topic/row ')]))"/>
		<!--GEHC.Count the current row-->
		<xsl:variable name="actual-row-count" select="number(count(../preceding-sibling::*[contains(@class, ' topic/row ')])+1)"/>
		<!--GEHC.Count the total rows-->		
		<xsl:variable name="total-row-count" select="$preceding-row-count + $following-row-count + 1"/>
		<!--GEHC.Add actual and total row counts to number second entry in row-->
		<xsl:variable name="second-entry-count" select="$actual-row-count + $total-row-count"/>
		<!--GEHC.Count the number of preceding entry elements-->
		<xsl:variable name="precedingEntry">
            <xsl:value-of select="count(preceding-sibling::*[contains(@class,' topic/entry ')])"/>
        </xsl:variable>
	    <xsl:choose>
		    <!--GEHC.Output "head" entry elements like headings-->
				<xsl:when test="@outputclass='head'">
				    <fo:block xsl:use-attribute-sets="thead.row.entry__content">
                        <xsl:call-template name="processEntryContent"/>
                    </fo:block>
				</xsl:when>
			<!--GEHC.Check if the table is FigLegend3Col-->
			<xsl:when test="ancestor::*[contains(@class, ' topic/table ')]/@outputclass='FigLegend3Col'">
                    <xsl:choose>
					    <!--GEHC.If the entry is the first in the row, insert a number-->
						<xsl:when test="$precedingEntry = 0">					
					        <fo:block xsl:use-attribute-sets="tbody.row.entry__content">
							    <xsl:call-template name="insertVariable">
                                    <xsl:with-param name="theVariableID" select="'Table Number'"/>
                                    <xsl:with-param name="theParameters">
                                        <number>
											<xsl:value-of select="$actual-row-count"/>
                                        </number>
                                    </xsl:with-param>
                                </xsl:call-template>
							</fo:block>
					    </xsl:when>
					    <!--GEHC.If the entry is not the first in the row, just process its content-->
						<xsl:otherwise>
						    <fo:block xsl:use-attribute-sets="tbody.row.entry__content">
                                <xsl:call-template name="processEntryContent"/>
                            </fo:block>
			...
			<!--GEHC.Check if the table is FigLgd2Col_FullPg-->
			<xsl:when test="ancestor::*[contains(@class, ' topic/table ')]/@outputclass='FigLgd2Col_FullPg'">
                    <xsl:choose>
					    <!--GEHC.If the entry is the first in the row, insert the actual row count-->
						<xsl:when test="$precedingEntry = 0">					
							<fo:list-block provisional-distance-between-starts="8mm" provisional-label-separation="1mm" space-after="1pt" space-before="1pt">
                                <fo:list-item font-size="9pt" margin-left="2pt" margin-right="2pt">
                                    <fo:list-item-label end-indent="label-end()" keep-together.within-line="always" keep-with-next.within-line="always">
                                        <fo:block font-weight="normal" text-align="left">										
										    <xsl:choose>
											    <xsl:when test="xml:lang='none'">
												    <!--Don't insert a number-->
												</xsl:when>
												<xsl:otherwise>
												    <xsl:call-template name="insertVariable">
                                                        <xsl:with-param name="theVariableID" select="'Table Number'"/>
					    ...
					    <!--GEHC.If the entry is not the first in the row, insert the second entry count-->
						<xsl:otherwise>
						    <fo:list-block provisional-distance-between-starts="8mm" provisional-label-separation="1mm" space-after="1pt" space-before="1pt">
                                <fo:list-item font-size="9pt" margin-left="2pt" margin-right="2pt">
                                    <fo:list-item-label end-indent="label-end()" keep-together.within-line="always" keep-with-next.within-line="always">
										    ...
										    <xsl:choose>
												...
												<xsl:otherwise>
												    <xsl:call-template name="insertVariable">
                                                        ...
                                                        <xsl:with-param name="theParameters">
                                                            <number>
                                                                <xsl:value-of select="$second-entry-count"/>
                                                            </number>
                                                        </xsl:with-param>
                                                    </xsl:call-template>
												</xsl:otherwise>
			...
			<!--GEHC.Check if the table is FigLgd2Col_InCol-->
			<xsl:when test="ancestor::*[contains(@class, ' topic/table ')]/@outputclass='FigLgd2Col_InCol'">
                    <xsl:choose>
					    <!--GEHC.If the entry is the first in the row, insert the first entry count-->
						<xsl:when test="$precedingEntry = 0">					
					        <fo:list-block provisional-distance-between-starts="8mm" provisional-label-separation="1mm" space-after="1pt" space-before="1pt">
                                <fo:list-item font-size="9pt" margin-left="2pt" margin-right="2pt">
                                    <fo:list-item-label end-indent="label-end()" keep-together.within-line="always" keep-with-next.within-line="always">
            ...
            <!--GEHC.Check if the table is NColNoLines-->
		    <xsl:when test="ancestor::*[contains(@class, ' topic/table ')]/@outputclass='NColNoLines'">
					<fo:block xsl:use-attribute-sets="tbody.row.entry__content">
                        <xsl:call-template name="processEntryContent"/>
                    </fo:block>
            </xsl:when>
			<!--GEHC.Check if the table is NColLines-->
		    <xsl:when test="ancestor::*[contains(@class, ' topic/table ')]/@outputclass='NColLines'">
				    <fo:block xsl:use-attribute-sets="tbody.row.entry__content">
                        <xsl:call-template name="processEntryContent"/>
                    </fo:block>
            </xsl:when>
            <!--GEHC.Check if the table is Alarm4Col-->
			<xsl:when test="ancestor::*[contains(@class, ' topic/table ')]/@outputclass='Alarm4Col'">
                    <fo:block xsl:use-attribute-sets="tbody.row.entry__content">
                        <xsl:call-template name="processEntryContent"/>
                    </fo:block>
            </xsl:when>	
			<!--GEHC.Check if the table is Parts-->
			<xsl:when test="ancestor::*[contains(@class, ' topic/table ')]/@outputclass='Parts'">
                    <fo:block xsl:use-attribute-sets="tbody.row.entry__content">
                        <xsl:call-template name="processEntryContent"/>
                    </fo:block>
            </xsl:when>
			<!--GEHC.Check if the table is Menu. This table type is deprecated. There is no special processing for it.-->
			<xsl:when test="ancestor::*[contains(@class, ' topic/table ')]/@outputclass='Menu'">
                    <fo:block xsl:use-attribute-sets="tbody.row.entry__content">
                        <xsl:call-template name="processEntryContent"/>
                    </fo:block>
            </xsl:when>	
			<!--GEHC.Check if the table is MenuSettings. This table type is deprecated. There is no special processing for it.-->
			<xsl:when test="ancestor::*[contains(@class, ' topic/table ')]/@outputclass='MenuSettings'">
                    <fo:block xsl:use-attribute-sets="tbody.row.entry__content">
                        <xsl:call-template name="processEntryContent"/>
                    </fo:block>
            </xsl:when>	
            <!--GEHC.Handle any other tables-->
			<xsl:otherwise>
                    <fo:block xsl:use-attribute-sets="tbody.row.entry__content">
                        <xsl:call-template name="processEntryContent"/>
                    </fo:block>
