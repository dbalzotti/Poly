...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="tableAttrs" select="'../attrs/tables-attr.xsl'"/>

    <!--GEHC. Changed to true to force actual column widths in tables-->
                    ...
                    <number>
						...
						<xsl:choose>
							<xsl:when test="count(ancestor-or-self::*[contains(@class, ' topic/topic')][position()=last()][count(preceding-sibling::*[contains(@class, ' topic/topic')]) &gt; 0])">
								<xsl:value-of select="count(./preceding::*[contains(@class, ' topic/table ')][child::*[contains(@class, ' topic/title ')]][ancestor-or-self::*[contains(@class, ' topic/topic')][position()=last()]]) - count(ancestor-or-self::*[contains(@class, ' topic/topic')][position()=last()]/preceding-sibling::*[contains(@class, ' topic/topic')]//*[contains(@class, ' topic/table ')][child::*[contains(@class, ' topic/title ')]])+1"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="count(./preceding::*[contains(@class, ' topic/table ')][child::*[contains(@class, ' topic/title ')]][ancestor-or-self::*[contains(@class, ' topic/topic')][position()=last()]])+1"/>
							</xsl:otherwise>
						</xsl:choose>
                    </number>
                    <title>
    ...
    <xsl:template match="*[contains(@class, ' topic/tgroup ')]" name="tgroup">
        ...
        <xsl:variable name="table">
            <!--GEHC. The fo:table used to call an attribute set with overrides based on @pgwide. This did not work. Removed call to attr set and replaced with
			xsl:choose below-->
			<fo:table>
                <xsl:call-template name="commonattributes"/>
                ...
                <!--GEHC. Added to control table span based on @pgwide value on table element-->
				<xsl:choose>
