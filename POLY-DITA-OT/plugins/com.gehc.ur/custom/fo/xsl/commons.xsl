...
-->
    ...
    <xsl:template match="*" mode="placeNoteContent">
        ...
        <fo:block xsl:use-attribute-sets="note">
			...
			<xsl:call-template name="commonattributes"/>
                ...
                <xsl:call-template name="place-note-image"/> <!-- PBG: Added 20170807 -->
                            <xsl:call-template name="insertVariable">
    ...
    <fo:block line-height="0pt" clear="both"/> <!-- PBG: Added 20170807 -->
  </xsl:template>

  <xsl:template name="place-note-image"> <!-- PBG: Added 20170807 -->
    ...
    <xsl:if test="not($noteImagePath = '')">
      <fo:inline-container alignment-adjust="hanging">
        <fo:block>
          <fo:external-graphic src="url({concat($artworkPrefix, $noteImagePath)})">
	    <xsl:attribute name="content-width">scale-to-fit</xsl:attribute>
            <xsl:attribute name="width">30pt</xsl:attribute>
          </fo:external-graphic>
        </fo:block>
      </fo:inline-container>
      <fo:leader leader-length="-8pt"/> <!-- Kerns image closer to admontion label word -->
    </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="*[contains(@class,' topic/note ')]">
        ...
        <xsl:choose>
      <xsl:when test="not($noteImagePath = '') and false()"> <!-- PBG: As of 20170807, we handle note images via place-note-image -->
