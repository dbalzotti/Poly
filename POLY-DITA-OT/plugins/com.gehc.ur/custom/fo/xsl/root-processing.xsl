...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:opentopic-i18n="http://www.idiominc.com/opentopic/i18n" xmlns:opentopic-index="http://www.idiominc.com/opentopic/index" xmlns:opentopic="http://www.idiominc.com/opentopic" xmlns:opentopic-func="http://www.idiominc.com/opentopic/exsl/function" exclude-result-prefixes="opentopic-index opentopic opentopic-i18n opentopic-func" version="1.1">
  ...
  <!-- get-doc-ishfield-value gets a Publication Document Version Property
  ...
  <!--GEHC. Get the part number from bookmap or notices topic-->
  <xsl:variable name="bookPartNo">
    <!--GEHC: Use this variable definition if the ditamap uses the data element for the part number-->
    <xsl:variable name="NoticesPartno" select="//*[contains(@class, ' topic/p ') and @outputclass='bookpartno']"/>
    <!--GEHC: use this variable definition if the bookmap uses the bookid/bookpartno element for the part number-->
    <xsl:variable name="bkmapPartNo" select="//*[contains(@class,' topic/data bookmap/bookpartno')][1]"/>
    <xsl:choose>
      <xsl:when test="$NoticesPartno">
        <xsl:value-of select="$NoticesPartno"/>
      </xsl:when>
      <xsl:when test="$bkmapPartNo">
        <xsl:value-of select="$bkmapPartNo"/>
      </xsl:when>
      <xsl:otherwise>
  ...
  <!--GEHC. Get the rev number from bookmap or notices topic-->
  <xsl:variable name="revNo">
    <!--GEHC: Use this variable definition if the ditamap uses the data element for the part number-->
    <xsl:variable name="NoticesRevno" select="//*[contains(@class, ' topic/p ') and @outputclass='revNo']"/>
    <!--GEHC: use this variable definition if the bookmap uses the bookid/bookpartno element for the part number-->
    <xsl:variable name="bkmapRevNo" select="//*[contains(@class,' topic/data bookmap/bookpartno')][1]/@rev"/>
    <xsl:choose>
      <xsl:when test="$NoticesRevno">
        <xsl:value-of select="$NoticesRevno"/>
      </xsl:when>
      <xsl:when test="$bkmapRevNo">
        <xsl:value-of select="$bkmapRevNo"/>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:variable name="optionalTitle" select="//*[contains(@class,' topic/data ') and @name = 'optional_title'][1]"/>


  <!--GEHC. Get the combined part number, language, and Rev number-->
  <xsl:variable name="fullPartNumber">
	<xsl:value-of select="$bookPartNo"/>
	<xsl:value-of select="$printedLang"/>
    <xsl:text> </xsl:text>
    <xsl:call-template name="insertVariable">
        <xsl:with-param name="theVariableID" select="'Rev Label'"/>
    </xsl:call-template> 
	<xsl:text> </xsl:text>
    <xsl:value-of select="$revNo"/>
  </xsl:variable>   
  ...
  <xsl:variable name="monthYear" select="format-dateTime(current-dateTime(),'[M01] [Y01]')"/>
  ...
  <xsl:template name="rootTemplate">
    ...
    <fo:root xsl:use-attribute-sets="__fo__root">
      ...
      <xsl:choose>
        <xsl:when test="(//*[contains(@class, ' map/map ')][@outputclass = 'nofmatter'])"> </xsl:when>
        <xsl:when test="(//*[contains(@class, ' map/map ')][@outputclass = 'nomatter'])"> </xsl:when>
        <xsl:otherwise>
      ...
      <xsl:choose>
        <xsl:when test="(//*[contains(@class, ' map/map ')][@outputclass = 'nobmatter'])"> </xsl:when>
        <xsl:when test="(//*[contains(@class, ' map/map ')][@outputclass = 'nomatter'])"> </xsl:when>
        <xsl:otherwise>
